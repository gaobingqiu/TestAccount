...
public class CircularStatisticsView extends View {
     ...
     * 第一圈圆环的颜色
     */
    private int mRingColor = Color.parseColor("#D8D8D8");
    /**
     * 第二圈的颜色
     */
    private int mSectorColor = Color.parseColor("#FBA923");
    /**
     * 终点扇形角度
     */
    ...
    protected void onDraw(Canvas canvas) {
        ...
        LogUtil.d(String.valueOf(centerX) + "&&" + centerY);
        // 计算外圆和内圆的半径
        if (centerX < centerY) {
            mOuterRadius = centerX;// 外圆半径
        } else {
            mOuterRadius = centerY;
        }
        ...
        LogUtil.d(String.valueOf(mOuterRadius));
        ...
        float baseline = (centerY * 2 - fontMetrics.bottom - fontMetrics.top) / 2;
        canvas.drawText(mText + "%", centerX, baseline, mTextPaint);
    }
     ...
     * 设置当前进度
     */
    public void setPercentage(float rang) {
        mSweepAngle = (rang) * 360f;
        mEndAngle = mStartAngle + mSweepAngle;
        mText = String.valueOf(Math.round(rang * 100));
        postInvalidate();
    }
}
